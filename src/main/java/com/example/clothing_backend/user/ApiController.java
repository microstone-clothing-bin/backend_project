...
//            return ResponseEntity.status(403).body(Map.of("status", "error", "message", "게시글을 수정할 권한이 없습니다."));
//        }
//
...
//
//        Board board = boardService.getBoard(boardId);
//        // LoginInfo 대신 User 객체와 getRoles()를 직접 사용
...
//        } else {
//            return ResponseEntity.status(403).body(Map.of("status", "error", "message", "게시글을 삭제할 권한이 없습니다."));
//        }
//    }
//
//    @PostMapping("/boards")
...
public class ApiController {
    ...
    @PostMapping("/boards")
    public ResponseEntity<Map<String, String>> writeBoard(@RequestParam String title, @RequestParam String content, @RequestParam(required = false) MultipartFile image, HttpSession session) throws IOException {
        User loginUser = (User) session.getAttribute("loginUser");
        if (loginUser == null) {
            return ResponseEntity.status(401).body(Map.of("status", "error", "message", "로그인이 필요합니다."));
        }
