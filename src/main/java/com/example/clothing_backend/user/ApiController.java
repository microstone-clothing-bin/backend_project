...
public class ApiController {
    ...
    public ResponseEntity<Map<String, String>> uploadProfile(
        ...
        try {
            // VVV 수정 VVV: UserService는 String 타입의 로그인 ID를 받으므로 loginUser.getId()를 전달합니다.
    ...
    public ResponseEntity<Map<String, String>> resetPassword(
        ...
        // VVV 수정 VVV: UserService는 String 타입 id와 email을 받으므로, 그에 맞게 전달합니다.
    ...
    // 현재 로그인 상태 확인 API
    // VVV 수정 VVV: 클래스에 /api가 이미 있으므로 중복되는 /api를 제거합니다.
    ...
    // 게시글 작성 시 받아오는 마커 API
    @GetMapping("/markers")
    public List<MarkerDto> getAllMarkers() {
        List<ClothingBin> bins = clothingBinService.findAll();
        return bins.stream()
                .map(bin -> new MarkerDto(bin.getLatitude(), bin.getLongitude(), bin.getRoadAddress()))
                .collect(Collectors.toList());
    }

    // --- 게시판 API --- (이하 코드는 문제 없음, 그대로 유지)
